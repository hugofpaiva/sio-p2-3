...
class MediaServer(resource.Resource):
    ...
    def encryptor(self, algorithm, mode, key, data, encryptor=None):
        if encryptor:
            if mode == 1:
                padder = padding.PKCS7(CHUNK_SIZE).padder()
    ...
    def decryptor(self, algorithm, mode, key, data, iv):
    ...
    def post_key(self, request):
        ...
        MESSAGE_KEY = HKDF(
            algorithm=hashes_options[CLIENT_INFO[request.client.host]['options']['selected_hash']](),
            length=32,
            salt=None,
    ...
    def do_get_protocols(self, request):
        options = json.dumps(CLIENT_INFO[request.client.host]['options']).encode('latin')

        options_digest = self.digest(CLIENT_INFO[request.client.host]['digest_key'],CLIENT_INFO[request.client.host]['options']['selected_hash'], options)
    ...
    def do_download(self, request):
        ...
        with open(os.path.join(CATALOG_BASE, media_item['file_name']), 'rb') as f:
            ...
            data, encryptor_cypher = self.encryptor(CLIENT_INFO[request.client.host]['options']['selected_algorithm'], CLIENT_INFO[request.client.host]
                                                    ['options']['selected_mode'], CLIENT_INFO[request.client.host]['message_key'], data, encryptor_cypher, CHUNK_SIZE)
            ...
            return json.dumps(
                {
                    ...
                    'digest': binascii.b2a_base64(digest_data).decode('latin').strip()
                }, indent=4
            ).encode('latin')
